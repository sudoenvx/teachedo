...
import { StatisticCard, Card } from "@teachedo/ui/legacy";
import { useAdminStats } from "../api/dashboard.queries";
import { cn } from "cn";
    ...
    <div className="flex flex-col gap-4 animate-in fade-in duration-300 pb-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 md:gap-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
        <StatisticCard
          ...
          value={
            statsLoading ? "..." : stats?.activeTeachers.toLocaleString() || "0"
          }
          icon={School}
        ...
        <StatisticCard
          ...
          value={
            statsLoading
              ? "..."
              : stats?.enrolledStudents.toLocaleString() || "0"
          }
          icon={GraduationCap}
        ...
        <StatisticCard
          ...
          value={
            statsLoading ? "..." : stats?.activeGroups?.toLocaleString() || "0"
          }
          icon={ActivitySquare}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="flex justify-between items-start mb-6">
            <div className="flex flex-col">
              <span className="flex items-center gap-1.5 text-[14px] font-bold text-text mb-1">
                <PieChart size={16} className="text-text-muted" /> الإيرادات
                الشهرية
              </span>
              <span className="text-[11px] text-text-muted">
                الإيرادات المحصلة بالمحفظة (MRR)
              </span>
            </div>
          ...
          <div className="flex-1 flex items-end justify-between gap-2 sm:gap-4 pt-4 border-b border-border px-2 pb-1 relative min-h-55">
            ...
            <div className="absolute inset-0 flex flex-col justify-between pointer-events-none z-0">
              {[400, 300, 200, 100, 0].map((val) => (
                <div
                  key={val}
                  className="flex items-center w-full border-t border-dashed border-border/60"
                >
                  <span className="text-[10px] font-medium text-text-faint absolute -right-3">
                    {val}
                  </span>
                </div>
              ))}
            ...
            {MRR_CHART_DATA.map((data, i) => (
              <div
                key={i}
                className="flex-1 flex flex-col items-center justify-end h-full group z-10"
              >
                <div
                  className={cn(
                    ...
                    data.type === "projected" &&
                      "bg-transparent border-2 border-dashed border-border group-hover:bg-surface-raised",
                  )}
                  style={{ height: `${data.value}%` }}
                >
                  <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-secondary-hover text-white text-[10px] font-bold px-2 py-1  opacity-0 group-hover:opacity-100 transition-opacity">
                    {data.value}K
                  </span>
                </div>
                <span className="text-[10px] font-semibold text-text-muted mt-2">
                  {data.month}
                </span>
              </div>
            ))}
        ...
        <Card>
          <div className="flex justify-between items-start mb-6">
            <div className="flex flex-col">
              <span className="flex items-center gap-1.5 text-[14px] font-bold text-text mb-1">
                <TrendingUp size={16} className="text-text-muted" /> أفضل
                المدرسين (VIP)
              </span>
              <span className="text-[11px] text-text-muted">
                إجمالي طلاب أفضل 4 مدرسين
              </span>
            </div>
            ...
            {VIP_TEACHERS.map((teacher, i) => (
              <div key={i} className="flex flex-col gap-1.5">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[11px] font-bold text-primary font-inter">
                    {teacher.students}
                  </span>
                </div>
